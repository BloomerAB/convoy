...
// Suspend sets spec.suspend=true on a Flux resource.
func (f *ClusterFactory) Suspend(ctx context.Context, r model.Resource) error {
	return f.setSuspend(ctx, r, true)
}

// Resume sets spec.suspend=false on a Flux resource.
func (f *ClusterFactory) Resume(ctx context.Context, r model.Resource) error {
	return f.setSuspend(ctx, r, false)
}

func (f *ClusterFactory) setSuspend(ctx context.Context, r model.Resource, suspend bool) error {
	cc, ok := f.Client(r.Cluster)
	if !ok {
		return fmt.Errorf("cluster %s not found", r.Cluster)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("%s %s/%s on %s: %w", action, r.Namespace, r.Name, r.Cluster, err)
	}
	return nil
}

func gvrForKind(kind model.ResourceKind) (schema.GroupVersionResource, error) {
	switch kind {
